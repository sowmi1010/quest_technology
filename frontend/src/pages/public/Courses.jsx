...
function SkeletonCard() {
  return (
    <div className="overflow-hidden rounded-3xl border border-peacock-border/60 bg-white/70 shadow-soft backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/35">
      <div className="h-44 animate-pulse bg-peacock-bg dark:bg-white/10" />
      <div className="p-5">
        <div className="h-3 w-24 animate-pulse rounded bg-peacock-bg dark:bg-white/10" />
        <div className="mt-2 h-5 w-3/4 animate-pulse rounded bg-peacock-bg dark:bg-white/10" />
        <div className="mt-4 space-y-2">
          <div className="h-4 w-2/3 animate-pulse rounded bg-peacock-bg dark:bg-white/10" />
          <div className="h-4 w-1/2 animate-pulse rounded bg-peacock-bg dark:bg-white/10" />
          <div className="h-4 w-3/5 animate-pulse rounded bg-peacock-bg dark:bg-white/10" />
        </div>
        <div className="mt-5 h-10 animate-pulse rounded-2xl bg-peacock-bg dark:bg-white/10" />
      </div>
    </div>
  );
}
    ...
    <PremiumShell>
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        ...
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="group relative overflow-hidden rounded-3xl border border-peacock-border/60 bg-white/70 p-6 shadow-soft backdrop-blur-xl md:p-8 dark:border-white/10 dark:bg-slate-950/35"
        >
          {/* glows */}
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
            {/* Search */}
            <div className="flex items-center gap-2 rounded-2xl border border-peacock-border/60 bg-white/60 px-3 py-2 shadow-soft dark:border-white/10 dark:bg-white/5">
          ...
          <div className="mt-4 text-xs text-peacock-muted dark:text-white/60">
            Showing{" "}
            <span className="font-extrabold text-peacock-navy dark:text-white">{totalShown}</span>{" "}
        ...
        {loading ? (
          ...
          <motion.div
            ...
            className={`mt-6 grid gap-5 ${view === "grid" ? "sm:grid-cols-2 lg:grid-cols-3" : "grid-cols-1"}`}
          >
...
function CourseCard({ course, index, compact }) {
  ...
  return (
    <motion.article
      ...
      className="group relative overflow-hidden rounded-3xl border border-peacock-border/60 bg-white/70 shadow-soft backdrop-blur-xl transition hover:shadow-xl dark:border-white/10 dark:bg-slate-950/35"
    ...
    >
      ...
      <div className={compact ? "grid md:grid-cols-[220px_1fr]" : ""}>
          ...
          {img ? (
            <div className="relative overflow-hidden">
              <img
                src={img}
                alt={course.title}
                className={`${compact ? "h-44 md:h-full" : "h-44"} w-full object-cover transition duration-500 group-hover:scale-[1.04]`}
                loading="lazy"
              />
            ...
            <div
              className={`${
                compact ? "h-44 md:h-full" : "h-44"
              } flex items-center justify-center bg-peacock-bg text-peacock-muted dark:bg-white/5 dark:text-white/60`}
            >
        ...
        <div className="p-5">
          ...
          <h2 className="mt-2 text-lg font-extrabold tracking-tight text-peacock-navy dark:text-white">
            {course.title}
          </h2>
          ...
          <div className="mt-3 space-y-2 text-sm text-peacock-muted dark:text-white/60">
            ...
            <p className="flex items-center gap-2">
              ...
              <span className="font-semibold">
                Total Fee:{" "}
                <span className="font-extrabold text-peacock-navy dark:text-white">
